...
def devicehistory_view_location(request, location):
    url = reverse('devicehistory-view')
    return redirect(url+'?loc=%s' % location)
...
def devicehistory_view_room(request, room):
    url = reverse('devicehistory-view')
    return redirect(url+'?room=%s' % room)
